...
// Mongo shell'den çıkış yap:
exit // Shell'den çıkış yapar.
quit // Shell'den çıkış yapar.

/* Veritabanları */
show('dbs') // Mevcut veritabanlarını gösterir.
// Veritabanı oluştur veya mevcut veritabanına geçiş yap:
use('newdb') // 'newdb' isimli veritabanını kullanmaya başlar (yoksa oluşturur).
// Veritabanını sil veya sıfırla:
db.dropDatabase() // Mevcut veritabanını siler.

/* Koleksiyonlar (Tablolar) */
show('collections') // Mevcut koleksiyonları gösterir.
db.getCollectionNames() // Koleksiyon isimlerinin listesini getirir.
db.getCollectionInfos() // Koleksiyon bilgilerinin listesini getirir.
db.createCollection('collName') // 'collName' isimli yeni bir koleksiyon oluşturur.
db.collName.renameCollection('collName2') // 'collName' isimli koleksiyonu 'collName2' olarak yeniden adlandırır.
db.collName2.drop() // 'collName2' isimli koleksiyonu siler.

/* Dokümanlar (Kayıtlar/Satırlar) */
...
// db.coll.insert() yöntemi kullanımdan kaldırılmıştır.

/* SEÇİM */
// Tek bir dokümanı filtrelerle birlikte bulma:
db.coll.findOne({ firstName: 'Test' })
// Filtrelerle dokümanları bulma:
db.coll.find({ firstName: 'Test' })
// Belirli alanları seçerek dokümanları bulma:
db.coll.find({}, { _id: false, firstName: true, lastName: true })
// Benzersiz 'firstName' değerlerini bulma:
db.coll.distinct('firstName')
// Karşılaştırma operatörleri ile dokümanları bulma (örneğin: $eq, $ne, $gt, $gte, vs.):
db.coll.find({ 'age': { $gt: 15 } }) // Yaşı 15'ten büyük olanları bulur.
// Regex ile dokümanları bulma:
db.coll.find({ firstName: /Test/ }) // 'Test' içeren isimleri bulur.
// Mantıksal operatörler ile dokümanları bulma (örneğin: $and, $or, $nor):
db.coll.find({ $or: [{ 'firstName': 'Test6' }, { 'age': 15 }] }) // OR operatörü ile filtreleme.
// Limit ve sıralama ile dokümanları bulma:
db.coll.find().sort({ age: -1 }).limit(5)
// Doküman sayma:
db.coll.countDocuments({ firstName: 'Test' })

// GÜNCELLEME:
// Tek bir dokümanı güncelleme:
db.coll.updateOne({ age: 19 }, { $set: { new_fields: 'new_value' } })
// Birden fazla dokümanı güncelleme:
db.coll.updateMany({ age: 19 }, { $set: { new_fields: 'new_value3' } })
// db.coll.update() yöntemi kullanımdan kaldırılmıştır.

// SİLME:
// Tek bir dokümanı silme:
db.coll.deleteOne({ age: 19 })
// Birden fazla dokümanı silme:
db.coll.deleteMany({ age: 19 })
// db.coll.remove() yöntemi kullanımdan kaldırılmıştır.""